...
const countries = [
  ...
  "GA - Gabon", "GM - Gambia", "GH - Ghana", "GN - Guinea", "GW - Guinea-Bissau", "KE - Kenya",
  "LS - Lesotho", "LR - Liberia", "LY - Libya", "MG - Madagascar", "MW - Malawi", "ML - Mali",
  ...
  "FR - France", "GE - Georgia", "DE - Germany", "GR - Greece", "HU - Hungary", "IS - Iceland",
  ...
  "SG - Singapore", "KR - South Korea", "LK - Sri Lanka", "SY - Syria", "TJ - Tajikistan",
...
const AuthModal: React.FC<AuthModalProps> = ({ type, open, onClose }) => {
 ...
 const isOldEnough = () => {
  ...
  // Check if any date parts are missing or if dobYear is before 1990
  if (!dobDay || !dobMonth || !dobYear || parseInt(dobYear) < 1990) return false;
  ...
  const validateForm = () => {
    ...
    if (authMode === "register") {
      if (
        ...
        !country || !agree || !securityAnswer
      ) {
        return "All fields are required.";
      }

      if (!isOldEnough()) return "You must be at least 16 years old to register.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const baseUrl = "https://www.gigpesa.co.ke";
    const url =
      authMode === "register"
    ...
    try {
      ...
      if (contentType.includes("application/json")) {
        ...
        data = await res.text();
        console.warn("Expected JSON, got text:", data);
      }
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-full sm:max-w-md max-h-[90vh] overflow-y-auto px-6 py-6 rounded-2xl shadow-lg bg-white">
        ...
        <ScrollArea className="h-full">
         <form onSubmit={handleSubmit} className="space-y-6 px-4 sm:px-6 py-6">
            {isRegister && (
              <>
    ...
    {showCountryDropdown && (
      <div className="relative border rounded-md p-2">
        <Input
          ...
          onChange={(e) =>
            handleChange("countrySearch", e.target.value.toLowerCase())
          }
        />
          ...
          <div className="flex flex-col">
              ...
              .map((c) => (
                <button
                  ...
                  className={`text-left px-3 py-2 hover:bg-gray-100 ${
                    form.country === c ? "bg-gray-00 font-medium" : ""
                  }`}
                  onClick={() => {
            ...
            {(authMode === "signin" || isRegister) && (
              <div className="space-y-2">
                ...
                {isRegister && (
                  <p className="text-xs text-gray-500 mt-1">
                    Must be 8+ characters, include uppercase, lowercase, and a number.| Must be 16 years plus to Register.
                  </p>
                )}
              </div>
            )}
